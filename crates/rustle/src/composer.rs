...
const FORMAT_COMMANDS: [(&str, &str); 6] = [
    ("bold", "bold"),
    ("italic", "italic"),
    ("underline", "underline"),
...
impl ComposerWindow {
    ...
    pub fn for_mailto(
        ...
        let signature = settings::signature_text(settings);
    ...
    fn build_from_row(&self, account: &Account) {
        ...
        let emails: Vec<&str> = accounts.iter().map(|each| each.email.as_str()).collect();
        imp.from_row.set_model(Some(&gtk::StringList::new(&emails)));
        let index = accounts
            .iter()
    ...
    fn build_editor(&self) {
        ...
        manager.connect_script_message_received(
            ...
            glib::clone!(
                #[weak(rename_to = window)]
                self,
                move |_, value| window.on_editor_changed(&value.to_str())
            ),
        );
        ...
        // Transparent, so the Adwaita "card" behind it supplies the
        // background and the editor tracks the theme without hardcoding.
        ...
        let page = EDITOR_PAGE
            ...
            .replace("__BODY__", &imp.body_html.borrow())
            .replace(
                "__COMMANDS__",
    ...
    fn on_editor_changed(&self, json: &str) {
        let Ok(payload) = serde_json::from_str::<EditorPayload>(json) else {
            return;
        };
        let imp = self.imp();
